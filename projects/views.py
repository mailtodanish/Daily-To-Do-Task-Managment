...
from .forms import LovCreateForm, ProjectTaskCreateForm
from .forms import ProjectTaskCommentCreateForm
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            messages.error(request, f"User has been created but wait for \
            approval.")
            context = {"create_user": form}
            return render(request,
                          "projects/registration/sign_up.html", context)
        else:
            context = {"create_user": form}
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['tasks'] = queryset = Activity.objects.filter(
            scheduled__lte=dt, Type='General').exclude(
                status='Done').order_by('scheduled')[0:5] 
        return context
...
class TaskDetail(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['comments'] = TaskComment.objects.filter(
            task__slug=self.kwargs.get("slug")).order_by('-pk')
        absolute_url = self.request.build_absolute_uri(ProjectTask.objects.get(
...
def get_shareable_link(request, signed_slug):
    ...
    try:
        ...
        absolute_url = request.build_absolute_uri(ProjectTask.objects.get(
            slug=slug).get_absolute_url())
        context = {
            'comments': TaskComment.objects.filter(
                task__slug=slug).order_by('-pk'),
